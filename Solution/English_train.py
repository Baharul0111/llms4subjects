...
def main():
    ...
    english_csv_path = "/path to English database"
    df_english = pd.read_csv(english_csv_path, low_memory=False)
    print("English dataset loaded. Rows:", len(df_english))

    subject_names_corpus = df_english['Name'].fillna('').astype(str).tolist()
    ...
    train_csv_path = "/path to train data"
    ...
    print("Building M embeddings (Title + Abstract) ...")
    ...
    print("Starting  Soft Retrieval Training with Attention")
